...
router.get('/hieu_chinh/:id', (req, res) => {
    var id = req.params.id;
    Promise.all([productModel.updatesingle(id), catModel.allChildren(), tagModel.singelByBaiViet(id),imgModel.getImgByProduct(id)]).then(([rows, cat, tag,img]) => {
        var strTag = "";
    ...
    if (!req.file) {
        ...
        Promise.all([productModel.update(entityProduct), imgModel.update(entityHinh)]).then(([product, img]) => {
            ...
            Promise.all([bar, imgProModel.update(imgProEntity)]).then(([result, id]) => {
                console.log(id);
            }).then(() => { res.redirect('/admin/hieu_chinh_bai_viet'); }
            ).catch(next);
